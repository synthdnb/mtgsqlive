...
def create_db(database_connection):
    ...
    cursor.execute(
        "create table luColors (colId integer PRIMARY KEY, colorName text);")
    cursor.execute(
    ...
    cursor.execute(
        "create table luTypes (tId integer PRIMARY KEY, typeName text);")
    cursor.execute(
    ...
    cursor.execute(
        "create table luBorders (bId integer PRIMARY KEY, borderName text);")
    cursor.execute(
        "create table luLegalities (legId integer PRIMARY KEY, legalityName text);")
    cursor.execute(
        "create table luFormats (fId integer PRIMARY KEY, formatName text);")
    cursor.execute(
        "create table luSets (setId integer PRIMARY KEY, setName text, setCode text, setReleaseDate text);")
    cursor.execute(
        "create table luLanguages (langId integer PRIMARY KEY, languageName text);")
    ...
    cursor.execute(
        "create table cForeignNames (cfnId integer PRIMARY KEY, cId integer, langId integer, name text, multiverseid text);")
    cursor.execute(
        "create table cRulings (crlId integer PRIMARY KEY, cId integer, date text, text text);")
    cursor.execute(
        "create table cVariations (cvId integer PRIMARY KEY, cId integer, vId integer);")
    # Create Views
...
def get_db_id(table_name, table_column_name, table_id_column, lookup_value, cursor):
    ...
    if table_name == "lnkLegalityFormat":
        ...
        format_id = get_db_id('luFormats', 'formatName',
                              'fID', legality_rule['format'], cursor)
    ...
    if len(returned_values) == 1:
        ...
        if table_name == "lnkLegalityFormat":
            cursor.execute(
                "INSERT INTO lnkLegalityFormat (legId, fId) VALUES (?, ?);", (legality_id, format_id))
        else:ㅈ
            cursor.execute("INSERT INTO {0} ({1}) VALUES (?);".format(
                table_name, table_column_name), (lookup_value,))
...
def load_cards(json_data, db_connection):
    ...
    for card_set_code in set_names:
        ...
        set_data = json_data[card_set_code]

        card_set_name = set_data["name"]
        set_release_date = set_data["releaseDate"]
        cursor.execute("INSERT INTO luSets (setName, setCode, setReleaseDate) VALUES (?, ?, ?);",
        ...
        for card_in_set in cards_in_set:
            ...
            card_layout_id = get_db_id('luLayouts', 'layoutName', 'layId', get_value_from_key(
                card_in_set, "layout"), cursor)
            card_name = get_value_from_key(card_in_set, "name")

            card_additional_names_list = get_value_from_key(
                card_in_set, "names")
            if card_additional_names_list:
                card_additional_names = " // ".join(card_additional_names_list)
            else:
                card_additional_names = None
            ...
            card_rarity_id = get_db_id('luRarities', 'rarityName', 'rId', get_value_from_key(
                card_in_set, "rarity"), cursor)
            ...
            card_multiverse_id = get_value_from_key(
                card_in_set, "multiverseid")
            card_image_name = get_value_from_key(card_in_set, "imageName")
            card_watermark = get_value_from_key(card_in_set, "watermark")
            card_border_id = get_db_id('luBorders', 'borderName', 'bId', get_value_from_key(
                card_in_set, "border"), cursor)
            card_time_shifted = bool(
                get_value_from_key(card_in_set, "timeshifted"))
            ...
            card_original_text = get_value_from_key(
                card_in_set, "originalText")
            card_original_type = get_value_from_key(
                card_in_set, "originalType")
            ...
            card_data = [
                ...
                card_multiverse_id, card_image_name, card_watermark, card_border_id,
                card_time_shifted, card_hand, card_life, card_reserved, card_release_date,
                card_starter, card_original_text, card_original_type, card_source,
                set_db_id, card_mci_id
            ]
            ...
            if card_legalities:
                for legality in card_legalities:
                    cursor.execute("INSERT INTO lnkCardLegFormat (cId, llfId) VALUES (?, ?);", (
                        card_db_id, get_db_id('lnkLegalityFormat', '', '', json.dumps(legality), cursor)))

            # Link printings
            card_printings = get_value_from_key(card_in_set, "printings")
            if card_printings:
                for printing in card_printings:
                    cursor.execute("INSERT INTO lnkCardPrinting (cId, pId) VALUES (?, ?);", (
                        card_db_id, get_db_id('luPrintings', 'printingName', 'pID', printing, cursor)))

            # Link colors
            card_colors = get_value_from_key(card_in_set, "colors")
            if card_colors:
                for color in card_colors:
                    cursor.execute("INSERT INTO lnkCardColor (cId, colId) VALUES (?, ?);", (
                        card_db_id, get_db_id('luColors', 'colorName', 'colId', color, cursor)))

            # Link color identities
            card_color_identities = get_value_from_key(
                card_in_set, "colorIdentity")
            if card_color_identities:
                for colorIdentity in card_color_identities:
                    cursor.execute("INSERT INTO lnkCardColorIdentity (cId, ciId) VALUES (?, ?);", (card_db_id, get_db_id(
                        'luColorIdentities', 'colorIdentityName', 'ciId', colorIdentity, cursor)))

            # Link supertypes
            card_supertypes = get_value_from_key(card_in_set, "supertypes")
            if card_supertypes:
                for supertype in card_supertypes:
                    cursor.execute("INSERT INTO lnkCardSupertype (cId, suptId) VALUES (?, ?);", (card_db_id, get_db_id(
                        'luSupertypes', 'supertypeName', 'suptId', supertype, cursor)))

            # Link types
            card_types = get_value_from_key(card_in_set, "types")
            if card_types:
                for type in card_types:
                    cursor.execute("INSERT INTO lnkCardType (cId, tId) VALUES (?, ?);", (
                        card_db_id, get_db_id('luTypes', 'typeName', 'tId', type, cursor)))

            # Link subtypes
            card_subtypes = get_value_from_key(card_in_set, "subtypes")
            if card_subtypes:
                for subtype in card_subtypes:
                    cursor.execute("INSERT INTO lnkCardSubtype (cId, subtId) VALUES (?, ?);", (
                        card_db_id, get_db_id('luSubtypes', 'subtypeName', 'subtId', subtype, cursor)))

            # Link foreign names
            card_foreign_names = get_value_from_key(
                card_in_set, "foreignNames")
            if card_foreign_names:
                for foreign_name in card_foreign_names:
                    ...
                    multiverseid = get_value_from_key(
                        foreign_name, "multiverseid")
                    cursor.execute("INSERT INTO cForeignNames (cId, langId, name, multiverseid) VALUES (?, ?, ?, ?);", (
                        card_db_id, get_db_id('luLanguages', 'languageName', 'langId', language, cursor), name, multiverseid))

            card_rulings = get_value_from_key(card_in_set, "rulings")
            if card_rulings:
                for ruling in card_rulings:
                    date = get_value_from_key(ruling, "date")
                    text = get_value_from_key(ruling, "text")
                    cursor.execute(
                        "INSERT INTO cRulings (cId, date, text) VALUES (?, ?, ?);", (card_db_id, date, text))

            card_variations = get_value_from_key(card_in_set, "variations")
            if card_variations:
                for variation in card_variations:
                    cursor.execute(
                        "INSERT INTO cVariations (cId, vId) VALUES (?, ?);", (card_db_id, variation))
